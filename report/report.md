...
- **Структура проекта:** код разделён на модули `src/preprocessing.py`, `src/eda.py`, `src/modeling.py`, общие пути и seed вынесены в `src/config.py`. Отчёт, графики, данные и модели лежат в отдельных директориях.
- **Воспроизводимость:** все разбиения, sampling и стохастические модели используют фиксированный `RANDOM_STATE = 42`.
- **Зависимости:** список библиотек и минимальных версий задан в `requirements.txt`.
- **Линтеры:** настроены `ruff` в `pyproject.toml` и `flake8` в `.flake8`.
- **Тесты:** smoke-тесты пайплайна находятся в `tests/test_pipeline.py` и проверяют создание целевой переменной, признаков и корректность split.
- **CI:** workflow `.github/workflows/ci.yml` запускает `ruff`, `flake8` и `pytest`.
- **Docker:** есть `Dockerfile` и `docker-compose.yml`, которые позволяют запустить экспериментальный пайплайн в контейнере.
...
- Был построен полный пайплайн для задачи регрессии: загрузка двух платформ, очистка, feature engineering, стратифицированный split, EDA, эксперименты и сохранение финальной модели.
- Baseline на минимальных признаках показал слабое качество: `MAE(test) = 68.185`.
- Feature engineering оказался решающим: уже линейная модель на структурных признаках снизила `MAE(test)` до `5.773`.
- Лучшей моделью стал `RandomForestRegressor`: после финального переобучения на `train + val` он получил `MAE(test) = 0.055`.
- Эксперимент с `TF-IDF + TruncatedSVD` показал, что уменьшение размерности для текстовых признаков в этой задаче ухудшает качество, потому что компактное представление теряет значимую информацию.
- Главное ограничение постановки: длина сообщения почти детерминированно восстанавливается из текстовых структурных признаков, поэтому задача больше проверяет корректность пайплайна и экспериментов, чем сложное языковое моделирование.
- Возможные улучшения: проверить постановку без признаков, напрямую связанных с длиной, добавить признаки пунктуации/эмодзи/ссылок при наличии сырых текстов, а также сравнить результаты отдельно для Twitter и Reddit.
